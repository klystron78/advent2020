...
namespace {
    ...
    struct cube {
        ...
        cube(dim_t z, dim_t y, dim_t x, bool active):z(z), y(y), x(x), active(active) {}
    };
    ...
    using space_t = boost::multi_index_container<
        ...
        mi::indexed_by<
            ...
            mi::hashed_non_unique<
                mi::member<cube, bool, &cube::active>
            >
        >
    >;

    int count_neighbors(const space_t &space, dim_t z, dim_t y, dim_t x)
    {
        int n=0;
        ...
        for(dim_t dz=(z-1); dz<=(z+1); ++dz)
            for(dim_t dy=(y-1); dy<=(y+1); ++dy)
                for(dim_t dx=(x-1); dx<=(x+1); ++dx) {
                    if((dz==z)&&(dy==y)&&(dx==x)) continue; /* don't check self */
                    auto it = space.find(std::make_tuple(dz, dy, dx));
    ...
    {
        const int n = count_neighbors(space, c.z, c.y, c.x);
        bool na=false;

        if(n==3) na=true;
        else if(c.active&&n==2) na=true;

        output.emplace(c.z, c.y, c.x, na);
    }

    void check_and_emplace(const space_t &space, space_t &output, dim_t z, dim_t y, dim_t x)
    {
        check_and_emplace(space, output, {z, y, x, false});
    }
        ...
        for(dim_t z=(min_z-1); z<=(max_z+1); ++z) {
            for(dim_t y=(min_y-1); y<=(max_y+1); ++y) {
                for(dim_t x=(min_x-1); x<=(max_x+1); ++x) {
                    check_and_emplace(space, output, z, y, x);
                }
            }
        }
        return output;
...
int main()
{
    ...
    const auto &idx = space.get<1>();
    auto ret = idx.equal_range(true);
    int total=0;
